...
int contaNodi(List*);
int counter(List*);
void updateFile(List*, List*, char*);
void writeLog();
//int nOcc(List*);
int count [2] = {0,0};
...
if (optind < argc)
	{
		...
		free(files);
		//List* v =StampaFile(outputFile, l); 
...
/* void updateFile(List* v, List* l, char* outputFile){
	//comparare le liste
	...
	while(l->next != NULL){
		while(v->next != NULL){
			if(strcmp(l->parola,v->parola)==1){
				i = l->occorrenza; y = v->occorrenza;
				fprintf(fp, "%s", l->parola);
				if(l<v){ fprintf(fp, "%d", l->occorrenza-v->occorrenza);}
				else{ fprintf(fp, "%d", (v->occorrenza)-(l->occorrenza)); }
			}else{v = v->next;}
		l=l->next;
		}
